// ==UserScript==
// @name         Tweakers Selective Quote
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  Tweakers Selective Quote
// @author       Trickster
// @match        https://gathering.tweakers.net/*
// @grant        none
...
function initComplete() {
    console.log("Selective Quote Loaded");
    checkQuotePage();
...
function getQuote() {
    ...
    if (window.getSelection) {
        var selectedData = {};
        selectedData.selectedText = window.getSelection().toString();
        selectedData.messageID =  window.getSelection().anchorNode.parentNode.parentNode.parentNode.getAttribute('data-message-id');

        // console.log(selectedData);

        if (selectedData.messageID == undefined){
            alert("Bericht niet gevonden \n - een Quote selecteren is nog niet mogelijk ");
        }
        else {
            Ajax.sendRequest(location.protocol + "//" + location.host + "/ajax/get_quote/" + selectedData.messageID, {
                method: "GET",
                type: "json",
                async: !0,
                handler: function(e) {
                    var fullQuoteMessage = e.data.message_content_quoted;

                    selectedData.fullMessage = fullQuoteMessage;
                    sessionStorage.setItem('selectedQuote',JSON.stringify(selectedData));
                    returnQuote(selectedData);


                    window.location.href = location.protocol + "//" + location.host + "/forum/quote_message/" + selectedData.messageID + "#selectedQuote"
                }
            })
        }
...
function checkQuotePage(){
    ...
    console.log(hash);
    if (hash == '#selectedQuote') {
        var selectedQuoteRaw = sessionStorage.getItem("selectedQuote");
        var selectedQuoteData = JSON.parse(selectedQuoteRaw);

        // var text = selectedQuoteData['fullMessage'].replace(/[b]/, '[b] NEW B [/b]');

        // console.log(JSON.parse(selectedQuoteRaw));
        console.log(selectedQuoteData);
        var reg = new RegExp(/\[\/b\](.|\s)*\[\/quote\]/);
        var match = selectedQuoteData.fullMessage.match(reg);
        var orgMsg = match[0];
        var msg = selectedQuoteData.fullMessage.replace(match[0],'[/b]\n' + selectedQuoteData.selectedText + "[/quote]");
        // console.log(msg);
        // console.log(text);
